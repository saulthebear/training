...
# representing a sequence that begins with start and is length elements long. In the sequence, every element should be
# 3 times the previous element. Assume that the length is at least 1.

def triple_sequence(start, length)
  sequence = [start]

  (length - 1).times { sequence << sequence[-1] * 3 }
