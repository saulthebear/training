...
def multi_map(arr, n = 1, &prc)
  mapped = arr.clone
  n.times { mapped = mapped.map { |ele| prc.call(ele) } }
  mapped
end
...
def proctition(arr, &prc)
  ...
  arr.each do |ele|
    if prc.call(ele)
      new_arr.insert(first_false_index, ele)
      first_false_index += 1
    else
      new_arr << ele
    end
...
def selected_map!(arr, selector_prc, changer_prc)
  arr.map! do |ele|
    if selector_prc.call(ele)
      changer_prc.call(ele)
    else
      ele
    end
  end
...
def chain_map(val, prcs)
  prcs.reduce(val) { |new_val, prc| prc.call(new_val) }
end
...
def proc_suffix(sentence, hash)
  ...
  sentence.split.each do |word|
    changed_word = word
    hash.each { |prc, suffix| changed_word += suffix if prc.call(word) }
...
def procipher(sentence, hash)
  ...
  sentence.split.each do |word|
    changed_word = word
    hash.each { |key_prc, val_prc| changed_word = val_prc.call(changed_word) if key_prc.call(word) }
    new_words << changed_word
  end
  ...
  sentence.split.each do |word|
    ...
    hash.each do |key_prc, val_prc|
      if key_prc.call(word)
        changed_word = val_prc.call(changed_word)
        break
      end
    end
    new_words << changed_word
