...
class Board
  ...
  # Where is it legal to place a mark? All positions that are in-bounds and not containing a mark
  # @return [Array] all positions of the board that are legal to place a new mark on
  def legal_positions
    positions = []
    (0...@board_width).each do |row_idx|
      (0...@board_width).each do |col_idx|
        positions << [row_idx, col_idx] if empty?([row_idx, col_idx])
      end
    end
    positions
  end
