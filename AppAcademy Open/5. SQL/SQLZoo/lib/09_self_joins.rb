  ...
  execute(<<-SQL)
    ...
    FROM
      ...
      routes AS end_routes
        ON start_routes.company = end_routes.company
          AND
          start_routes.num = end_routes.num
      JOIN
        stops AS start_stops
        ON start_stops.id = start_routes.stop_id
      JOIN
        stops AS end_stops
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    SELECT
      from_craiglockhart.num,
      from_craiglockhart.company,
      to_sighthill.name,
      to_sighthill.num,
    ...
    FROM
      ...
      FROM
        ...
        JOIN
          ...
          ON end_stops.id = end_routes.stop_id
      WHERE
        start_stops.name = 'Craiglockhart') AS from_craiglockhart
      JOIN
      (
        SELECT DISTINCT
          start_stops.name,
      ...
      ON
      to_sighthill.name = from_craiglockhart.name
  SQL
end
