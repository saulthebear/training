// Original tests refactored to use Mocha & Chai
...
describe('Fixed tests', function() {
  it('It should pass fixed tests', function() {
    assert.equal(songDecoder('AWUBBWUBC'),
        'A B C', 'WUB should be replaced by 1 space');
    assert.equal(songDecoder('AWUBWUBWUBBWUBWUBWUBC'),
        'A B C', 'multiples WUB should be replaced by only 1 space');
    assert.equal(songDecoder('WUBAWUBBWUBCWUB'),
